...
map='set_2'
pc_indices <- c(1, 4, 6, 7, 8, 10, 12, 14, 21) # set-1
if(map=='set_2') pc_indices <- c(2, 3, 4, 6, 7, 10, 12, 14, 19, 22, 26) # set-2, last 3 need to be made

pc_indices <- c(19, 22, 26)
...
for(i in 1:length(rnk_files)){
  ...
  analysis_name = gsub('.rnk', '',list.files(rnk_file_path)[i])
  print(analysis_name)
  working_dir = working_dir
  GSEA_command = paste("",gsea_jar,  "GSEAPreRanked -gmx", dest_gmt_file, "-rnk" ,
                       rnk_file , "-collapse false -nperm 1000 -scoring_scheme weighted -rpt_label ",
